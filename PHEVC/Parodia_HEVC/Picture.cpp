...
void Picture::loadFrameFromYuv( std::ifstream& yuvFile )
{
	...
	for( UInt i = 0; i < ch; ++i )
	{
		...
		{
			yuvFile.read( &tmp, 1 );
			itsSamplesCr[ j ][ i ] = tmp + 128;
		}
	}
}

Sample** Picture::samples( const ImgComp comp ) const
{
	if( comp == LUMA )
		return itsSamplesY;
	else if( comp == CR )
...
Sample** Picture::reconMatrix( const ImgComp comp ) const
{
	if( comp == LUMA )
		return itsReconY;
	else if( comp == CR )
